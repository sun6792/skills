...
public class Boss {
    ...
    private void postJobByCity(String cityCode) {
        ...
        for (String keyword : config.getKeywords()) {
            ...
            if (shouldStopCallback.get()) {
                progressCallback.accept("User cancelled", 0, 0);
                return;
            }
            ...
            // Check for BOSS redirect/verification immediately after navigation
            if (checkBossRedirect(page)) return;
            ...
            // 3. Process all jobs one by one
            Locator cards = page.locator("//ul[contains(@class, 'rec-job-list')]//li[contains(@class, 'job-card-box')]");
            int count = cards.count();
            for (int i = 0; i < count; i++) {
                // Check stop signal
                if (shouldStopCallback != null && Boolean.TRUE.equals(shouldStopCallback.get())) {
                    progressCallback.accept("User cancelled", i, count);
                    return;
                }

                // Check for BOSS redirect on every card (BOSS may detect automation mid-delivery)
                if (i > 0 && i % 5 == 0) { // Check every 5 cards to avoid overhead
                    if (checkBossRedirect(page)) return;
                }
                ...
                // Add human-like delay between cards (random 1-3 seconds)
                if (i > 0) {
    ...
    /**
     * Check if BOSS redirected to slider verification or login page.
     * Returns true if delivery should stop (slider appeared or we got logged out).
     */
    private boolean checkBossRedirect(Page page) {
        try {
            String currentUrl = page.url();
            // Check for slider verification
            if (currentUrl != null && currentUrl.contains("/web/user/safe/verify")) {
                log.warn("BOSS slider verification detected, pausing for manual solve...");
                progressCallback.accept("BOSS verification required - please solve the slider in the browser window", 0, 0);
                // Wait for user to solve (poll URL until it changes from verify page)
                long start = System.currentTimeMillis();
                while (System.currentTimeMillis() - start < 5 * 60 * 1000) {
                    PlaywrightUtil.sleep(3);
                    try {
                        String newUrl = page.url();
                        if (newUrl != null && !newUrl.contains("/web/user/safe/verify")) {
                            log.info("BOSS verification passed, resuming...");
                            progressCallback.accept("Verification passed, resuming delivery", 0, 0);
                            return false;
                        }
                ...
                log.error("BOSS verification timeout (5 minutes)");
                return true;
            }
            // Check for login page redirect (kicked out)
            if (currentUrl != null && (currentUrl.contains("/web/user/") || currentUrl.contains("login"))) {
                log.warn("BOSS redirected to login page - may have been logged out");
                progressCallback.accept("BOSS requires re-login. Please log in again in the browser window.", 0, 0);
                return true;
            }
        } catch (Exception e) {
            log.debug("Error checking BOSS redirect: {}", e.getMessage());
        }
        return false;
    }
    ...
    private void waitForSliderVerify(Page page) {
        ...
        while (true) {
            ...
            if (url != null && url.startsWith(SLIDER_URL)) {
                progressCallback.accept("Please solve the BOSS slider verification in the browser window", 0, 0);
                log.warn("BOSS slider verification required - waiting for manual solve...");
                PlaywrightUtil.sleep(3);
                continue;
            }
            if ((System.currentTimeMillis() - start) > 5 * 60 * 1000) {
                throw new RuntimeException("Slider verification timeout!");
            }
            break;
        }
    }
