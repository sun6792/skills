...
public class PlaywrightManager {
  ...
  private volatile boolean job51MonitoringPaused = false;
    ...
    // 防止 Boss 登录页重复跳转（避免 BOSS 检测到自动化后反复刷新）
    private volatile long lastBossLoginRedirectMs = 0L;
    private static final long BOSS_LOGIN_REDIRECT_COOLDOWN_MS = 30_000L; // 30秒冷却
  ...
  private static final String JOB51_URL = "https://www.51job.com";
    ...
    /**
     * Initialize Playwright - tries CDP connection first, falls back to launching Chrome.
     */
    public void init() {
        if (isInitialized()) {
            return;
        ...
        log.info("========================================");
        log.info("  Initializing browser automation engine");
        log.info("========================================");

        try {
            playwright = Playwright.create();
            log.info("Playwright engine started");

            // Use system Chrome with persistent profile
            // (No CDP --remote-debugging-port exposed - BOSS detects that)
            ...
            log.info("Initializing all platforms...");
            ...
            log.info("Browser automation engine initialized (all platforms started)");
            log.info("========================================");
        } catch (Exception e) {
            log.error("Browser automation engine init failed", e);
            throw new RuntimeException("Playwright init failed", e);
        }
    }

    /**
     * Launch system Chrome with persistent profile (no CDP port exposed).
     * The persistent profile keeps BOSS login state across restarts.
     * BOSS cannot detect this because there's no --remote-debugging-port flag.
     */
    private void launchBrowserWithPersistentProfile() {
        ...
        if (osName.contains("win")) {
            ...
            for (String c : candidates) {
                if (new java.io.File(c).exists()) { chromePath = c; break; }
            }
        }

        // Persistent profile directory (login state survives restarts)
        java.nio.file.Path profileDir = java.nio.file.Paths.get(
            System.getProperty("user.dir"), ".chrome-profile").toAbsolutePath();
        profileDir.toFile().mkdirs();
        log.info("Using persistent Chrome profile: {}", profileDir);

        // Critical: NO --remote-debugging-port (BOSS detects this!)
        java.util.List<String> args = new java.util.ArrayList<>(List.of(
            ...
            "--disable-features=ChromeWhatsNewUI,site-per-process"
        ));
        ...
        if (chromePath != null) {
            ...
            log.info("Launching system Chrome: {}", chromePath);
        }

        // launchPersistentContext returns BrowserContext directly (browser managed internally)
        context = playwright.chromium().launchPersistentContext(
            java.nio.file.Paths.get(profileDir.toString()), opts);
        browser = context.browser();
        log.info("Chrome launched with persistent profile (zh-CN locale)");
        injectAntiDetectionScripts(context);
    }

    /**
     * Try to connect to an existing Chrome via CDP (port 9222).
     * The user launches Chrome manually via start-chrome-cdp.bat with --remote-debugging-port=9222.
     */
    private boolean tryConnectOverCdp() {
        try {
            String cdpUrl = "http://localhost:9222";
            log.info("Trying CDP connection to {}...", cdpUrl);
            browser = playwright.chromium().connectOverCDP(cdpUrl);
            log.info("CDP connected successfully to existing Chrome!");

            // Use first existing context (the real Chrome profile)
            List<BrowserContext> contexts = browser.contexts();
            if (!contexts.isEmpty()) {
                context = contexts.get(0);
                log.info("Using existing BrowserContext (real Chrome profile)");
            } else {
            ...
            log.info("CDP connection failed (Chrome not running on port 9222): {}", e.getMessage());
            log.info("Falling back to launching new Chrome...");
            return false;
        }
    }

    /**
     * Launch a new Chrome browser (fallback when CDP not available).
     */
    private void launchNewBrowser() {
        ...
        java.util.List<String> chromeArgs = new java.util.ArrayList<>(List.of(
            "--disable-blink-features=AutomationControlled",
            "--start-maximized",
            "--no-first-run",
            "--no-default-browser-check"
        ...
        if (chromePath != null) {
            launchOpts.setExecutablePath(java.nio.file.Paths.get(chromePath));
            log.info("Launching system Chrome: {}", chromePath);
        }

        browser = playwright.chromium().launch(launchOpts);
        log.info("Chrome browser launched (standalone mode)");

        context = browser.newContext(new Browser.NewContextOptions().setViewportSize(null));
        log.info("BrowserContext created");
        injectAntiDetectionScripts(context);
    }

    /**
     * Create fresh pages for all 4 platforms (standalone mode).
     */
    private void createAllPages() {
        log.info("Creating pages for all platforms...");
        bossPage = context.newPage();
        bossPage.setDefaultTimeout(DEFAULT_TIMEOUT);
        log.info("BOSS Page created");

        liepinPage = context.newPage();
        liepinPage.setDefaultTimeout(DEFAULT_TIMEOUT);
        log.info("Liepin Page created");

        job51Page = context.newPage();
        job51Page.setDefaultTimeout(DEFAULT_TIMEOUT);
        log.info("51job Page created");

        zhilianPage = context.newPage();
        zhilianPage.setDefaultTimeout(DEFAULT_TIMEOUT);
        log.info("Zhilian Page created");
    }

    /**
     * Discover existing pages or create new ones (CDP mode).
     * In CDP mode, the user's existing tabs are preserved.
     */
    private void discoverOrCreatePages() {
        log.info("Discovering existing pages in CDP Chrome...");
        List<Page> existingPages = context.pages();
        log.info("Found {} existing pages", existingPages.size());

        // Try to match existing pages by URL, or create new ones
        for (int i = 0; i < existingPages.size(); i++) {
            Page p = existingPages.get(i);
            try {
                String url = p.url();
                log.info("Existing page {}: {}", i, url);
                if (bossPage == null && url.contains("zhipin.com")) {
                    bossPage = p;
                } else if (liepinPage == null && url.contains("liepin.com")) {
                    liepinPage = p;
                } else if (job51Page == null && url.contains("51job.com")) {
                    job51Page = p;
                } else if (zhilianPage == null && url.contains("zhaopin.com")) {
                    zhilianPage = p;
                }
        ...
        if (bossPage == null) {
            bossPage = context.newPage();
            log.info("Created new BOSS page");
        }
        bossPage.setDefaultTimeout(DEFAULT_TIMEOUT);

        if (liepinPage == null) {
            liepinPage = context.newPage();
            log.info("Created new Liepin page");
        }
        liepinPage.setDefaultTimeout(DEFAULT_TIMEOUT);

        if (job51Page == null) {
            job51Page = context.newPage();
            log.info("Created new 51job page");
        }
        job51Page.setDefaultTimeout(DEFAULT_TIMEOUT);

        if (zhilianPage == null) {
            zhilianPage = context.newPage();
            log.info("Created new Zhilian page");
        }
        zhilianPage.setDefaultTimeout(DEFAULT_TIMEOUT);
    }

    /**
     * Inject comprehensive anti-detection scripts into BrowserContext.
     * Covers navigator.webdriver, chrome.runtime, CDP detection, and plugin fingerprinting.
     */
    private void injectAntiDetectionScripts(BrowserContext targetContext) {
        // --- Script 1: Comprehensive anti-detection ---
        String coreAntiDetection = """
            (function(){
                try {
                    // 1. Hide navigator.webdriver (MUST be first)
                    var originalWebdriver = Object.getOwnPropertyDescriptor(Navigator.prototype, 'webdriver');
                    if (!originalWebdriver || originalWebdriver.get) {
                    ...
                    // 2. Fix window.outerWidth/outerHeight (Playwright sets to 0)
                    if (window.outerWidth === 0 || window.outerHeight === 0) {
                        Object.defineProperty(window, 'outerWidth', {get: function(){return screen.width;}});
                        Object.defineProperty(window, 'outerHeight', {get: function(){return screen.height - 80;}});
                    }

                    // 3. Fake chrome object (Boss checks this extensively)
                    window.chrome = {
                        runtime: {
                    ...
                    // 4. Fake plugins array
                    var makePlugins = function() {
                        var p = [
                    ...
                    // 6. Hardware concurrency (Playwright often sets to 1)
                    ...
                    // 8. Clean Playwright markers
                    delete window.__playwright;
                    delete window.__pw_manual;
                    delete window.__PW_inspect;
                    delete window.__pwInitScripts;

                    // 9. Override headless detection via userAgent
                    ...
                    // === CRITICAL: BOSS disable-devtool bypass ===
                    // BOSS uses console.table() timing + performance.now() to detect automation.
                    // These hooks prevent the timing-based detection from triggering.

                    // 10. Hook console.table (BOSS's primary detection vector)
                    var _origTable = console.table;
                    console.table = function() {
                        // Do nothing - BOSS checks if this takes >50ms to detect DevTools
                    };
                    // Hide the hook from toString detection
                    console.table.toString = function() { return 'function table() { [native code] }'; };

                    // 11. Hook performance.now to prevent timing-based detection
                    var _origNow = performance.now;
                    var _timeOrigin = performance.timeOrigin;
                    var _fixedTime = Date.now();
                    performance.now = function() {
                        // Return time based on Date.now() which is less precise, preventing
                        // BOSS from detecting micro-timing anomalies caused by CDP
                        return Date.now() - _timeOrigin;
                    };
                    performance.now.toString = function() { return 'function now() { [native code] }'; };

                    // 12. Hook all console methods to prevent toString detection
                    var _methods = ['log', 'debug', 'info', 'warn', 'error', 'dir', 'trace', 'group', 'groupEnd', 'time', 'timeEnd', 'assert', 'clear', 'count', 'countReset'];
                    _methods.forEach(function(m) {
                        try {
                            var orig = console[m];
                            if (orig.toString() !== 'function ' + m + '() { [native code] }') {
                                orig.toString = function() { return 'function ' + m + '() { [native code] }'; };
                            }
                        } catch(e) {}
                    });

                    // 13. Hide Function.prototype.toString modifications from detection
        ...
        // --- Script 2: Boss-specific anti-detection (from anti-detection.js resource) ---
        String bossScript = readResourceText(BOSS_INIT_SCRIPT_RESOURCE);
        String bossWrapped = "(function(){try{if(location&&location.origin===\"https://www.zhipin.com\"){"
                + "if(window.__bossAntiDetectInjected){return;}window.__bossAntiDetectInjected=true;"
                + (bossScript != null && !bossScript.isBlank() ? bossScript : "")
                + "}}catch(e){}})();";

        // Inject core anti-detection for ALL sites
        targetContext.addInitScript(coreAntiDetection);
        log.info("Core anti-detection script injected into Context");

        // Inject Boss-specific script
        if (bossScript != null && !bossScript.isBlank()) {
            targetContext.addInitScript(bossWrapped);
            log.info("Boss anti-detection script injected into Context: {}", BOSS_INIT_SCRIPT_RESOURCE);
        }
    }
...
//                        log.warn("智联招聘：打开二维码登录面板失败: {}", e.getMessage());
    ...
    public void setLoginStatus(String platform, boolean isLoggedIn) {
        ...
        if (previousStatus == null || previousStatus != isLoggedIn) {
            ...
            // Boss: redirect to login page when not logged in (with cooldown to prevent flashing)
            if ("boss".equals(platform) && !isLoggedIn) {
                try {
                    if (bossPage != null) {
                        long now = System.currentTimeMillis();
                        // Cooldown guard: don't redirect more than once per 30 seconds
                        if (now - lastBossLoginRedirectMs < BOSS_LOGIN_REDIRECT_COOLDOWN_MS) {
                            return;
                        }
                        ...
                        // Don't re-navigate if already on login page
                        if (currentUrl == null || !currentUrl.contains("/web/user/")) {
                            log.info("Redirecting Boss to login page...");
                            bossPage.navigate(BOSS_URL + "/web/user/?ka=header-login");
                            lastBossLoginRedirectMs = now;
                            try { Thread.sleep(800); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        }
