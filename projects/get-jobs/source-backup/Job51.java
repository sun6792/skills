...
public class Job51 {
    ...
    /**
     * Deliver all jobs on current page
     */
    private void deliverCurrentPage() {
        try {
            PlaywrightUtil.sleep(1);

            // Find all job checkboxes - try multiple selectors (51job changes these frequently)
            Locator checkboxes = page.locator("div.ick");
            if (checkboxes.count() == 0) {
                checkboxes = page.locator("div[class*='ick']");
            }
            if (checkboxes.count() == 0) {
                checkboxes = page.locator(".j_check, input[type='checkbox'], div.checkbox, span.checkbox, i.chk");
            }
            if (checkboxes.count() == 0) {
                log.warn("[51job] No checkboxes found on page, trying JS click on all job cards");
                // Fallback: JS click all possible selection elements
                try {
                    page.evaluate("() => { document.querySelectorAll('div.ick, div[class*=ick], .j_check, input[type=checkbox]').forEach(el => { try { el.click(); } catch(e) {} }); }");
                    checkboxes = page.locator("div.ick"); // re-check
                } catch (Exception ignored) {}
            }
            if (checkboxes.count() == 0) {
                log.warn("[51job] No checkboxes found on page, skipping");
                return;
            }

            // Find job titles and company names
            Locator titles = page.locator("[class*='jname text-cut'], [class*='jname'], a.jname, span.jname");
            Locator companies = page.locator("[class*='cname text-cut'], [class*='cname'], a.cname, span.cname");
            ...
            for (int i = 0; i < jobCount; i++) {
                ...
                try {
                    ...
                    // Use JS click to avoid element obstruction
                    checkbox.evaluate("el => el.click()");
                    selected++;

                    String title = i < titles.count() ? titles.nth(i).textContent() : "Unknown";
                    String company = i < companies.count() ? companies.nth(i).textContent() : "Unknown";
                    String jobInfo = company + " | " + title;
                    resultList.add(jobInfo);
                } catch (Exception e) {
                    log.debug("[51job] Failed to click checkbox {}: {}", i, e.getMessage());
                }
            }

            if (selected == 0) {
                log.warn("[51job] Failed to select any jobs");
                return;
            }
            ...
            // Click batch deliver button
            boolean batchClicked = clickBatchDeliverButton();

            PlaywrightUtil.sleep(3);

            if (batchClicked) {
                // Handle delivery success dialog
                handleDeliverySuccessDialog();
            }

            // Handle separate delivery dialog
            handleSeparateDeliveryDialog();

        } catch (Exception e) {
            log.error("[51job] deliverCurrentPage failed: {}", e.getMessage());
        }
    }

    /**
     * Click batch delivery button. Returns true if button was found and clicked.
     */
    private boolean clickBatchDeliverButton() {
        ...
        while (retryCount < 5) {
            try {
                ...
                // Try multiple selectors for the batch deliver button
                Locator batchBtn = null;
                String[][] btnSelectors = {
                    {"div.tabs_in button.p_but", "1"},           // original: 2nd button
                    {"div.tabs_in button.p_but", "0"},           // fallback: 1st button
                    {"button:has-text('批量投递')", "0"},
                    {"button:has-text('批量申请')", "0"},
                    {"span:has-text('批量投递')", "0"},
                    {"div.tabs_in button", "1"},                 // any 2nd button in tabs_in
                    {"div.tabs_in button", "0"},                 // any 1st button in tabs_in
                    {"button.p_but", "0"},                       // any p_but button
                };

                for (String[] sel : btnSelectors) {
                    try {
                        Locator parent = page.locator(sel[0]);
                        int idx = Integer.parseInt(sel[1]);
                        if (parent.count() > idx) {
                            batchBtn = parent.nth(idx);
                            if (batchBtn.isVisible()) {
                                log.debug("[51job] Found batch button: {}[{}]", sel[0], idx);
                                break;
                            }
                        }
                    } catch (Exception ignored) {}
                ...
                if (batchBtn != null) {
                    ...
                    // Check for daily limit toast immediately after click
                    for (int i = 0; i < 10; i++) {
                        try { Thread.sleep(200); } catch (InterruptedException ignored) {}
                        if (detectDailyLimitToast51job()) {
                            reachedDailyLimit = true;
                            log.warn("[51job] Daily delivery limit reached after batch click");
                            sendProgress("Daily delivery limit reached, task stopped", null, null);
                            return true; // button WAS clicked, but limit hit
                        }
                    }

                    return true;
                } else {
                    log.warn("[51job] Batch delivery button not found with any selector");
                    return false;
                }
            } catch (Exception e) {
                retryCount++;
    ...
    private void handleDeliverySuccessDialog() {
        try {
            ...
            // Handle delivery success/failure dialog
            Locator elDialogBody = page.locator(".el-dialog__body");
            if (elDialogBody.count() > 0) {
                String dialogText = elDialogBody.first().innerText();
                // Match multiple variations: 投递成功, 申请成功, delivery success, etc.
                boolean isSuccessDialog = dialogText != null && (
                    dialogText.contains("投递成功") || dialogText.contains("申请成功") ||
                    dialogText.contains("已成功投递") || dialogText.contains("投递完成"));
                if (isSuccessDialog) {
                    Integer successNum = null;
                    Integer failNum = null;
                    try {
                        java.util.regex.Matcher m1 = java.util.regex.Pattern.compile("(?:投递成功|申请成功|成功投递|已成功)\\D*(\\d+)").matcher(dialogText);
                        if (m1.find()) successNum = Integer.parseInt(m1.group(1));
                        java.util.regex.Matcher m2 = java.util.regex.Pattern.compile("(?:未投递|失败|未申请)\\D*(\\d+)").matcher(dialogText);
                        if (m2.find()) failNum = Integer.parseInt(m2.group(1));
                    } catch (Exception ignored) {}
                    log.info("[51job] Delivery result: success={}, failed={}", successNum, failNum);
                    sendProgress(String.format("Delivery: %s success, %s failed", successNum == null ? "?" : successNum, failNum == null ? "?" : failNum), null, null);

                    // Mark delivered jobs in DB
                    if (successNum != null && successNum > 0) {
                        try {
                            ...
                            // Fallback: if JSON interceptor didn't get IDs, scrape from page DOM
                            if (deliveredIds.isEmpty()) {
                                log.info("[51job] currentPageJobIds empty, falling back to DOM scrape");
                                deliveredIds = collectJobIdsOnPage();
                            }
                            if (!deliveredIds.isEmpty()) {
                                ...
                                log.info("[51job] Marked {} jobs as delivered", toMark.size());
                            } else {
                                log.warn("[51job] No jobIds available (JSON interceptor + DOM scrape both empty), cannot mark delivered");
                            }
                        } catch (Exception e) {
                            log.warn("[51job] Failed to mark delivered: {}", e.getMessage());
                        }
                    } else {
                        log.warn("[51job] Delivery success count is 0 or unparseable, skipping mark");
                    }

                    // Close dialog: try OK/confirm buttons, close icons, ESC
                    try {
                        Locator okBtn = page.locator(".el-dialog__footer button:has-text('确定'), .el-message-box__btns button:has-text('确定'), button:has-text('OK'), button:has-text('Confirm')");
                        if (okBtn.count() > 0) {
                            okBtn.first().click();
                        } else {
                            // 1) 点击关闭图标的父按钮
