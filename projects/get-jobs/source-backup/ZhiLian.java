...
public class ZhiLian {
        ...
        try {
            ...
            for (String keyword : config.getKeywords()) {
                if (shouldStop() || isLimit) {
                    sendProgress("Daily limit reached", null, null);
                    break;
                }
    ...
    private boolean deliverCurrentPage(String keyword) {
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                // Multi-selector fallback for company name (different job card layouts)
                String companyName = safeGetText(card, "div.companyinfo__name");
                if (companyName.isEmpty()) companyName = safeGetText(card, "a[class*='company'], div[class*='company'], span[class*='company']");
            ...
            for (PageJob pj : jobs) {
                ...
                // Multi-selector fallback for apply button (zhaopin.com changes class names)
                Locator applyBtn = card.locator("button.collect-and-apply__btn");
                if (applyBtn.count() == 0) {
                    applyBtn = card.locator("button:has-text('立即投递'), button:has-text('投递简历'), a:has-text('投递'), .apply-btn, [class*='apply']");
                }
                if (applyBtn.count() == 0) {
                    // Final fallback: find any button/link with delivery text
                    applyBtn = card.locator("button, a").filter(new Locator.FilterOptions().setHasText("投递|申请|立即"));
                }
                if (applyBtn.count() == 0) {
                    log.debug("Job [{}] no apply button found, skipping", pj.jobTitle);
                    continue;
                }

                // Check for CAPTCHA/verification before clicking (skip if false positive risk)
                // Only check CAPTCHA every 5 jobs to avoid false positives from normal page elements
                if (pj.index % 5 == 0 && detectCaptcha()) {
                    log.warn("Possible CAPTCHA detected, waiting 10s then retrying...");
                    PlaywrightUtil.sleep(10);
                    // Re-check: if still there, give up on this page
                    if (detectCaptcha()) {
                        log.warn("CAPTCHA still present, skipping rest of this page");
                ...
                try {
                    ...
                    try {
                        ...
                        // Wait briefly for success indicator
                        PlaywrightUtil.sleep(1);
                    } finally {
                        try { page.context().offPage(closer); } catch (Exception ignored) {}
                    }

                    // Verify delivery success before marking
                    boolean delivered = checkDeliverySuccess();
                    if (delivered) {
                        if (pj.jobId != null && !pj.jobId.isEmpty()) {
                            zhilianService.markDeliveredByJobId(pj.jobId);
                            log.info("Delivered: jobId={}, title={}, company={}", pj.jobId, pj.jobTitle, pj.companyName);
                        } else if (pj.jobTitle != null && pj.companyName != null) {
                            zhilianService.markDeliveredByTitleAndCompany(pj.jobTitle, pj.companyName);
                            log.info("Delivered: title={}, company={}", pj.jobTitle, pj.companyName);
                        }
                    } else {
                        log.debug("Delivery click done but no success indicator for: {}", pj.jobTitle);
                    }
                } catch (Exception clickEx) {
                    log.warn("Delivery click failed for [{}]: {}", pj.jobTitle, clickEx.getMessage());
                }
    ...
    /**
     * Check if daily delivery limit is reached
     */
        ...
        try {
            ...
            if (result.count() > 0) {
                ...
                if (text != null && text.contains("达到上限")) {
                    log.info("Daily delivery limit reached!");
                    isLimit = true;
                    return true;
                }
            }
    ...
    /**
     * Check if delivery was successful by looking for success indicators.
     */
    private boolean checkDeliverySuccess() {
        try {
            // Check for success toast/message
            String[] successIndicators = {"投递成功", "申请成功", "已投递", "已申请"};
            for (String indicator : successIndicators) {
                Locator el = page.locator("text=" + indicator);
                if (el.count() > 0 && el.first().isVisible()) {
                    return true;
                }
            }
            // Check for success class
            Locator successEl = page.locator(".deliver-success, .apply-success, .el-message--success, [class*='success']");
            if (successEl.count() > 0 && successEl.first().isVisible()) {
                return true;
            }
            // Check if button text changed to "已投递"
            Locator deliveredBtn = page.locator("button:has-text('已投递'), span:has-text('已投递')");
            if (deliveredBtn.count() > 0) {
                return true;
            }
            return false;
        } catch (Exception e) {
    ...
    /**
     * Detect CAPTCHA or anti-bot verification on page.
     */
    private boolean detectCaptcha() {
        try {
            ...
            String[] captchaSelectors = {
                ...
                "div.geetest_panel", "div.captcha", "div.verify", ".yidun",
                "div[class*='captcha']", "div[class*='verify']", "div[class*='slider']"
            };
            for (String sel : captchaSelectors) {
                if (page.locator(sel).count() > 0) {
                    log.warn("CAPTCHA detected via selector: {}", sel);
                    return true;
                }
            }
            // Check page title/URL for verification
            String url = page.url();
